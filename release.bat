...
if "%~1"=="" (
    echo [ERROR] Укажи версию для релиза!
...
go build -o apm.exe .
if %ERRORLEVEL% NEQ 0 (
    echo [ERROR] Ошибка сборки apm.exe
    exit /b 1
)
...
if %ERRORLEVEL% NEQ 0 (
    echo [ERROR] Ошибка сборки apm-installer.exe
    exit /b 1
)

echo [DEBUG] Проверка наличия GitHub CLI (gh)...
where gh >nul 2>nul
if %ERRORLEVEL% NEQ 0 (
    echo [ERROR] Утилита 'gh' (GitHub CLI) не найдена!
